...
\chapter{All about trees by Brian O'Meara}
...
Goals:
\begin{itemize}
\item Study the information content of phylogenetically structured data
\item Learn about particular tree formats in ape, phylobase, and ouch
\item Learn how to hand-make trees
\item Learn how to import trees from nexus and newick formats
\item Learn tree conversion from one format to another
\end{itemize}
Concepts:
\begin{itemize}
\item file access
...
In phylogenetics, we use ideas and terminology from both graph theory and nature. \textbf{Terminal taxa} are also known as \textbf{leaves}, \textbf{terminals}, \textbf{OTU}s ("\textbf{Operational Taxonomic Units}"), \textbf{tips}, or simply \textbf{taxa}. \textbf{Branches} are also called \textbf{edges}. \textbf{Internal} nodes (places where two or more branches connect) are also known as vertices and sometimes simply \textbf{nodes} (technically, leaves are also nodes). A \textbf{rooted} tree has one node designated as the \textbf{root}, and all other nodes are descended from this root. An \textbf{unrooted} tree has no root designated. Traditionally, the root node has at least two descendants; it may also have a subtending branch. A tree where every internal node has two and only two descendants is known as a \textbf{binary} or \textbf{bifurcating} tree. A tree where at least one internal node has more than two descendants is said to be \textbf{multifurcating}; such a node is a \textbf{polytomy}. Trees in phylogenetics generally represent either \textbf{species trees} (a history of the splitting of interbreeding populations) or \textbf{gene trees} (a history of the coalescence of gene copies). In both cases, it is generally believed that the true process is bifurcating, so that each split results in two descendants. Thus, polytomies on trees are generally taken as representing uncertainty in the relationships. Branches may have \textbf{lengths}; these lengths may correspond to time, amount of change in some set of characters, number of speciation events, or some other measure. A tree where all branch lengths from root to tips are equal is known as an \textbf{ultrametric} tree. A tree without branch lengths is known as a \textbf{topology}. A \textbf{clade} is an ancestor and all its descendants. Any \textbf{edge} corresponds to a \textbf{bipartition}: a division of the tree into two parts connected by that edge (if a root were inserted on the edge, then each of those parts would be a clade).
\subsection{Newick}
...
<<label=simpletreeplot,include=FALSE,echo=FALSE, fig=T>>=
...
\subsection{phylo (ape 1.9 or above)}
The \verb@ape@ package \citep{Paradis2004} uses a different representation of trees. It uses R \verb@structures@, \verb@lists@, \verb@matrices@, and \verb@vectors@ to store a tree. Each node in the tree receives a number. For example, here is the tree from before in \code{ape} format. First let's clear any old workspace, load our libraries, and load our tree called "\code{simpletree}" from "\code{Rdata/simpletree.rda}". 
<<eval=F>>=
rm(ls=list())
require(ape)
require(ouch)
...
<<label=simpletreeplotnodelabels,include=TRUE>>=
plot(simpletree,no.margin=TRUE)
nodelabels()
tiplabels()
@
...
@
\section{Getting trees into R}
There are ways to use R to estimate phylogenetic trees given a set of taxa with characters. For more information on this, see \citet{Paradis:2006}. In many cases, however, there will be trees saved in an existing file, saved as a result of an analysis in programs such as PAUP or MrBayes. This section will discuss getting those trees into R. Note there may be additional ways to load trees not discussed here. For example \verb@apTreeshape@ can load trees directly from \url{http://www.treebase.org} if you know the appropriate study number; as these trees lack branch lengths, they are generally unsuitable for the sort of analyses we will be doing in this course.
...
\section{Exercises}
\begin{enumerate}
\item Take \code{simpletree} in ape's phylo format and set all branch lengths equal to 1.0, plot to verify.
\item Convert \code{simpletree} to phylobase's phylo4 format. Which packages do you need in order to do this? Plot with the display nodes option (show.node=T).
\item Convert to ouch. Plot with displaying nodes (node.names=T). Convert to data frame representation. 
\item Assign node labels to your tree object. What is the easiest way to do that?
